...
def run_one_seed(
    ...
    val_loader = DataLoader(
        EEGDataset(X_val, y_val), batch_size=BATCH_SIZE, shuffle=False, num_workers=0
    )
    test_loader = DataLoader(
        EEGDataset(X_test, y_test), batch_size=BATCH_SIZE, shuffle=False, num_workers=0
    )
    ...
    print(
        f"  Training done in {train_time}s | Best Val F1={best_val_f1:.4f} | Best Val Acc={best_val_acc:.4f}"
    )
...
def run_seed_study() -> int:
    ...
    print(f"  Seeds      : {SEEDS}")
    print("  Frozen arch: CNN Baseline  (num_channels=64, num_classes=2)")
    print(
        f"  Max epochs : {MAX_EPOCHS}  |  batch_size={BATCH_SIZE}  |  ES patience={ES_PATIENCE} (exact match: benchmark line 287)"
    )
    print("  GAN aug    : DISABLED")
    ...
    for seed in SEEDS:
        result = run_one_seed(seed, X_train, y_train, X_val, y_val, X_test, y_test, meta, device)
    ...
    repro_report = {
        "study_timestamp": datetime.now(UTC).isoformat(),
    ...
    for s in SEEDS:
        ...
        print(
            f"  {s:>6}  {r['accuracy'] * 100:8.2f}%  {r['macro_f1']:9.4f}  {r['cohens_kappa']:8.4f}  {r['balanced_accuracy'] * 100:8.2f}%"
        )
    print(f"  {'─' * 6}  {'─' * 9}  {'─' * 9}  {'─' * 8}  {'─' * 9}")
    print(
        f"  {'MEAN':>6}  {agg['mean_test_accuracy'] * 100:8.2f}%  {agg['mean_macro_f1']:9.4f}  {agg['mean_cohens_kappa']:8.4f}  {agg['mean_balanced_accuracy'] * 100:8.2f}%"
    )
    print(
        f"  {'STD':>6}  {agg['std_test_accuracy'] * 100:8.2f}%  {agg['std_macro_f1']:9.4f}  {agg['std_cohens_kappa']:8.4f}  {agg['std_balanced_accuracy'] * 100:8.2f}%"
    )
    print("=" * 78)
