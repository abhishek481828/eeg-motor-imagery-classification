...
def check_01_paths() -> dict:
    ...
    if not ok:
        return _result(FAIL, "Dataset files missing",
                       {"npz": str(DATA_NPZ), "meta": str(DATA_META)})
    with open(DATA_META) as f:
        meta = json.load(f)
    return _result(PASS, "Dataset files confirmed",
...
def check_02_epoch_count(data: dict) -> dict:
    ...
    return _result(PASS if ok else FAIL,
                   ...
                   {"total": n, "train": int(len(data["X_train"])),
...
def check_05_subject_ids(meta: dict) -> dict:
    ...
    ok = (tr == list(range(1, 78))) and (val == list(range(78, 94))) and (te == list(range(94, 110)))
    return _result(PASS if ok else FAIL,
...
def check_06_no_overlap(meta: dict) -> dict:
    ...
    splits = meta.get("subject_splits", {})
    tr  = set(int(s) for s in splits.get("train",      []))
    val = set(int(s) for s in splits.get("validation", []))
    te  = set(int(s) for s in splits.get("test",       []))
    tv = tr & val
    tt = tr & te
    vt = val & te
    ok = len(tv) == 0 and len(tt) == 0 and len(vt) == 0
    return _result(PASS if ok else FAIL,
                   "No subject overlap detected" if ok else f"Overlap found! Train∩Val={tv}, Train∩Test={tt}, Val∩Test={vt}",
...
def check_07_no_duplicate_epochs(data: dict) -> dict:
    ...
    return _result(PASS if ok else FAIL,
                   "No duplicate epochs found" if ok else f"Duplicates: train∩val={tv}, train∩test={tt}, val∩test={vt}",
                   {"train_val_duplicates": tv,
                    "train_test_duplicates": tt,
                    "val_test_duplicates": vt})
...
def check_08_scaler_fitting(meta: dict) -> dict:
    ...
    if scaler_info:
        ...
        ok = fit_on == "train"
        return _result(PASS if ok else FAIL,
                       f"Scaler fit_on = '{fit_on}' (expected 'train')",
                       scaler_info)
    ...
    return _result(NOTE,
                   ...
                   {"code_path": "scripts/preprocess_full_dataset.py",
...
def check_09_val_only_selection() -> dict:
    ...
    return _result(PASS if ok else FAIL,
                   f"Best checkpoint at epoch {epoch} selected by val_macro_f1={val_f1:.4f}" if ok
...
def check_10_single_test_eval() -> dict:
    ...
    return _result(PASS if ok else FAIL,
                   f"Exactly one 'test_metrics' block confirmed" if ok
...
def check_12_prediction_distribution() -> dict:
    ...
    ok = len(pred_dist) >= 2
    return _result(PASS if ok else FAIL,
                   f"Prediction distribution: {pred_dist}" if ok
...
def check_13_recalculate_metrics(data: dict) -> dict:
    ...
    return _result(PASS if ok else FAIL,
                   "Recalculated metrics match saved report" if ok
...
def run_audit() -> int:
    ...
    print("=" * 78)
    print(f"  Audit started : {datetime.now(timezone.utc).isoformat()}")
