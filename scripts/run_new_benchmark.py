...
def run_experiment_1(X_tr, y_tr, X_v, y_v, X_te, y_te, meta) -> dict[str, Any]:
    ...
    # 1. Labels & Counts
    unique_labels = sorted(list(set(np.concatenate([y_tr, y_v, y_te]))))
    class_counts = {
    ...
    print(f"  ✓ Baseline Val Acc={v_metrics['accuracy']*100:.2f}%, Val Macro F1={v_metrics['macro_f1']:.4f}")
    return report
...
def run_experiment_2(X_tr, y_tr, X_v, y_v, device) -> list[dict[str, Any]]:
    ...
    for idx, cfg in enumerate(combinations, 1):
        ...
        model = create_model("eegnet", num_channels=64, num_classes=2, sequence_length=seq_len, dropout=cfg["dropout"])
        ...
        print(f"  [{idx:02d}/16] {name} | dropout={cfg['dropout']}, lr={cfg['lr']}, bs={cfg['batch_size']}, wd={cfg['weight_decay']} "
...
def run_experiment_3(X_tr, y_tr, X_v, y_v) -> list[dict[str, Any]]:
    ...
    for n_c in n_comp_list:
        ...
        print(f"  CSP+LDA n_components={n_c:02d} → Val Acc={v_metrics['accuracy']*100:.2f}%, Val F1={v_metrics['macro_f1']:.4f}")
...
def run_experiment_4(X_tr, y_tr, X_v, y_v) -> list[dict[str, Any]]:
    ...
    for b_name, l_freq, h_freq in bands:
        tr_b = mne.filter.filter_data(X_tr.copy().astype(np.float64), sfreq=sfreq, l_freq=l_freq, h_freq=h_freq, verbose=False)
        v_b  = mne.filter.filter_data(X_v.copy().astype(np.float64), sfreq=sfreq, l_freq=l_freq, h_freq=h_freq, verbose=False)
    ...
    tr_csp_feats = []
    v_csp_feats  = []
    for b_idx, (b_name, _, _) in enumerate(bands):
    ...
    for k in k_options:
        ...
        print(f"  FBCSP+LDA k_features={k} → Val Acc={v_metrics['accuracy']*100:.2f}%, Val F1={v_metrics['macro_f1']:.4f}")
...
def run_experiment_5(X_tr, y_tr, X_v, y_v, device) -> list[dict[str, Any]]:
    ...
    for idx, cfg in enumerate(grid, 1):
        ...
        print(f"  [{idx:02d}/{len(grid)}] {name} | filters={cfg['filters']}, k={cfg['kernel']}, drop={cfg['dropout']}, lr={cfg['lr']} "
              f"→ Val Acc={v_metrics['accuracy']*100:.2f}%, Val F1={v_metrics['macro_f1']:.4f}")
...
def main() -> int:
    ...
    exp1_rep = run_experiment_1(X_tr, y_tr, X_v, y_v, X_te, y_te, meta)
    ...
    if winner["exp_id"] == "frozen_baseline":
        ...
        if "eegnet" in winner["model_name"].lower():
            m_win = create_model("eegnet", num_channels=64, num_classes=2, sequence_length=X_tr.shape[2],
        ...
        bands = [("Theta", 4.0, 8.0), ("Alpha_Mu", 8.0, 12.0), ("Low_Beta", 12.0, 16.0), ("Mid_Beta", 16.0, 20.0), ("High_Beta", 20.0, 30.0)]
        sfreq = 160.0
        tr_b, te_b = [], []
        for b_name, l_freq, h_freq in bands:
            tr_b.append(mne.filter.filter_data(X_tr.copy().astype(np.float64), sfreq=sfreq, l_freq=l_freq, h_freq=h_freq, verbose=False))
            te_b.append(mne.filter.filter_data(X_te.copy().astype(np.float64), sfreq=sfreq, l_freq=l_freq, h_freq=h_freq, verbose=False))
        tr_feats, te_feats = [], []
