...
def run_phase_2(X_tr, y_tr, X_v, y_v, device) -> list[dict[str, Any]]:
    ...
    for b_name, l_freq, h_freq in bands:
        ...
        print(
            f"  {b_name:<10} → Val Acc={v_metrics['accuracy'] * 100:.2f}%, Val F1={v_metrics['macro_f1']:.4f}"
        )
    ...
    for n_c in csp_counts:
        ...
        class FusionDataset(torch.utils.data.Dataset):
            ...
            def __getitem__(self, idx):
                return self.raw[idx], self.csp[idx], self.y[idx]
        ...
        print(
            f"  CNN+CSP Fusion n={n_c:02d} → Val Acc={v_metrics['accuracy'] * 100:.2f}%, Val F1={v_metrics['macro_f1']:.4f}"
        )
...
def run_phase_4(X_tr, y_tr, X_v, y_v, device) -> list[dict[str, Any]]:
    ...
    for cfg in aug_configs:
        ...
        print(
            f"  {name:<15} → Val Acc={v_metrics['accuracy'] * 100:.2f}%, Val F1={v_metrics['macro_f1']:.4f}"
        )
...
def run_phase_5(X_v, y_v, device) -> dict[str, Any]:
    ...
    rec = {
        ...
        "total_parameters": sum(p.numel() for p in m1.parameters())
    ...
    print(
        f"  Ensemble (w_cnn={best_w:.2f}, w_eegnet={1 - best_w:.2f}) → Val Acc={best_ens_metrics['accuracy'] * 100:.2f}%, Val F1={best_ens_metrics['macro_f1']:.4f}"
    )
    return rec
...
def main() -> int:
    ...
    plt.axvline(
        ...
        color="red",
        linestyle="--",
        label=f"Tuned CNN Baseline ({TUNED_CNN_VAL_F1:.4f})",
    )
...
"""
    ...
    print("  ✓ STOPPING AT PHASE 7 BEFORE TEST EVALUATION — Awaiting user approval!")
    print("=" * 90 + "\n")
    return 0
